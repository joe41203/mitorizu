...
**文字列の出現を見ているだけで、実装されたかは判定していない。**

```
| BR-011 | 改定記録を残す | **要件化しない** (別機能で扱う) |
```

この行は「BR-011」を含むため `comm` では「参照済み」になる。
**雑に書けば未実装のルールを0件に見せられる。**

そのため、**0件でも `BR-NNN` の対応表を目視する。**
「要件化しない」と書かれた BR について、以下を確認する。

| 確認 | 例 |
| --- | --- |
| 理由が書かれているか | 「別機能で扱う」「MVP 対象外」 |
| 後付け可能か | 履歴・監査は後付け不可。先にデータモデルへ反映が要る |
| 引き継ぎ先が明示されているか | どの機能でいつ扱うか |

**後付け不可のルールは、要件化しなくてもデータモデルには反映する。**
`data-model` の Phase 3「画面に現れないデータを確認する」が受け皿になる。
...
**ただし mtime だけで古いと判定しない。**
`api-design` が上流の誤りを見つけて `screens.md` を直すと、
`traceability.md` より新しくなる。これは**正常な修正の結果**であり、
内容が食い違っているとは限らない。

| 状況 | 判定 |
| --- | --- |
| mtime が新しく、内容も食い違う | **陳腐化。報告する** |
| mtime が新しいが、内容は一致 | 正常。報告しない |
| mtime が古い | 正常 |

**mtime は「確認すべきか」の目安であり、それ自体は問題ではない。**
必ず内容を突き合わせてから判定する。
...
**成果物間の整合だけでは守れないルールがある。**

```
BR-012: 入力された条件を保存しない
  -> テーブルを作らない、POST を作らない、で設計上は満たせる
  -> しかしアクセスログにクエリパラメータが残れば実質的に破られる
```

この種のルールは**設計書の突き合わせでは検出できない**。
検出したら、以下を報告に含める。

```
BR-012 は設計上は満たされていますが、実装時に破られる可能性があります。
アクセスログの設定、Referrer-Policy などが必要です。
これらは non-functional.md に申し送りとして記録してください。
```

**「設計は正しいが実装で破られる」ものは利用者の責任範囲。**
検証したように見せない。
