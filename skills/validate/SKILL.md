...
```
REQ-002: システムは現在の借り手名を表示すること
  -> lendings テーブルに employee_id が無い
  -> 画面にも API にも「借り手名」が無ければ traceability は0件のまま通る
  -> しかし要件を満たせない
```

**要件を1つずつ読み、それを実現するデータが存在するかを確認する。**
特に以下は見落としやすい。

| 要件の記述 | 必要なデータ |
| --- | --- |
| 「〜を表示する」 | 表示する値のカラム、または導出元 |
| 「〜から N 日後」 | 起点となる日時のカラム |
| 「〜した人」 | 操作者を記録するカラム |
| 「〜の履歴」 | 履歴を残すテーブル、または論理削除 |
| 「〜を通知する」 | 通知先 (メールアドレス等) のカラム |
...
**以下のコマンドは実測で動作を確認済み。** パターンを変えるときは必ず
「検出されるべき例」と「検出されてはいけない例」の両方で試すこと。
