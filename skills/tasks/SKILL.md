...
### traceability が0件でも確認すること

**traceability は「画面 - API - テーブル」の3方向しか見ていない。**
要件との突合は含まれないため、以下を見逃す。

```
REQ-002: システムは現在の借り手名を表示すること
  -> lendings に employee_id が無い
  -> 画面にも API にも無ければ traceability は0件のまま通る
  -> しかし要件を満たせない
```

**タスク分解の前に、要件を1つずつ読み、実現に必要なデータが
`data-model.md` に存在するかを確認する。**

不足を見つけたら、タスク分解を続ける前に報告する。

```
REQ-002 の実現に必要な employee_id が lendings テーブルにありません。
traceability.md は0件ですが、これは画面・API・テーブル間の検証であり
要件との突合は含まれないためです。

A: data-model.md を修正してから続ける (推奨)
B: 不足を要確認項目に記録して続ける
```
...
**数値の集計は目視でなく機械計算する。** タスク数と見積合計は間違えやすい。

```bash
# タスク数 (表の行数)
grep -cE '^\| T-[0-9]+' docs/mitorizu/features/<slug>/tasks.md

# 見積の合計 (時間表記を分に換算して合算)
# 表のセルは前後に空白があるため、必ずトリムしてから判定する
awk -F'|' '/^\| T-[0-9]+/ {
  for (i=1; i<=NF; i++) {
    f=$i; gsub(/^[ \t]+|[ \t]+$/, "", f)
    if (f ~ /^[0-9]+分$/)   { gsub(/[^0-9]/,"",f); m += f }
    if (f ~ /^[0-9]+時間$/) { gsub(/[^0-9]/,"",f); m += f * 60 }
  }
} END { printf "合計: %d時間%d分\n", m/60, m%60 }' docs/mitorizu/features/<slug>/tasks.md
```

上記は実測で確認済み (30分 + 1時間 + 2時間 = 3時間30分)。
**トリムを省くと空白のせいで常に0になる。**

サマリに書いた数値と実際の表が食い違っていないかを、
**書いた直後に必ず突き合わせる。**
